...
import {Text,View,TouchableOpacity,StyleSheet,ScrollView,AsyncStorage,Share,Linking, UIManager, LayoutAnimation} from "react-native";
  ...
  */
  ...
  createThePhotos(){
    if(this.props.data.detailsSetup.showPhotos&&this.state.photos&&this.state.photos.length>0){
      ...
      return (<Photos onPress={this.handelPhotoClick} title={this.props.data.detailsSetup.photosTitle} photos={this.state.photos}/>)
    }else{
          ...
          <ScrollView contentContainerStyle={css.detailsScroll}>
            <View style={{flex: 1}}>
              <HeaderImage
                ...
                title={this.getExpectedValue('title',true,this.props.data.detailsSetup.fields.titleFunctions)}
              />
              <Text style={[css.static.intemInfoLabel,rtlText]}>{this.getExpectedValue('subtitle',true,this.props.data.detailsSetup.fields.subtitleFunctions)}</Text>
